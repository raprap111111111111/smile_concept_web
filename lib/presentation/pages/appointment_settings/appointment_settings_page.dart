...
import '../../providers/settings/appointment_settings_provider.dart';
import '../../theme/app_colors.dart';
import '../../theme/app_dimensions.dart';
import '../../theme/app_text_styles.dart';
import '../../theme/app_theme.dart';
import '../doctor_schedules/widgets/days_checkbox_list.dart';
import '../doctor_schedules/widgets/time_picker_field.dart';
import 'widgets/fee_field.dart';
import 'widgets/number_stepper_field.dart';
import 'widgets/settings_section_card.dart';

/// Admin screen for every configurable appointment rule.
///
/// One draft model, one atomic Save. Field edits flow into the provider's
/// draft; Save PUTs the whole form and the response becomes the new baseline.
class AppointmentSettingsPage extends ConsumerStatefulWidget {
  const AppointmentSettingsPage({super.key});

  @override
  ConsumerState<AppointmentSettingsPage> createState() =>
      _AppointmentSettingsPageState();
}

class _AppointmentSettingsPageState
    extends ConsumerState<AppointmentSettingsPage> {
  ...
  void initState() {
    ...
    _opensController.addListener(
        () => _onTimeEdited(_opensController, (d, t) => d.copyWith(clinicOpensAt: t)));
    _closesController.addListener(
        () => _onTimeEdited(_closesController, (d, t) => d.copyWith(clinicClosesAt: t)));
    _lunchStartController.addListener(
        () => _onTimeEdited(_lunchStartController, (d, t) => d.copyWith(lunchBreakStart: t)));
    _lunchEndController.addListener(
        () => _onTimeEdited(_lunchEndController, (d, t) => d.copyWith(lunchBreakEnd: t)));
  ...
  /// TimePickerField writes "HH:mm:ss"; the settings model keeps "HH:mm".
  void _onTimeEdited(
  ...
  Future<void> _save() async {
    ...
    if (saved && mounted) {
      // Server may have normalized values; keep the pickers in sync.
  ...
  /// Re-seed AFTER the response lands. Clearing the seeded flag up front would
  /// let the next build re-seed from the stale value and then latch, leaving
  /// the pickers showing the old times.
  ...
  Widget build(BuildContext context) {
    ...
    // Seed the time controllers exactly once, after the first load lands.
    if (!_controllersSeeded && state.saved != null) {
      _seedTimeControllers(state.saved!);
      // Set AFTER seeding so the listener skips these programmatic writes.
    ...
    // it pins the light theme the same way doctor_schedule_form_page does.
    return Theme(
  ...
  Widget _buildScaffold(BuildContext context, AppointmentSettingsState state) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Appointment Settings', style: AppTextStyles.titleLarge),
        centerTitle: false,
        actions: [
          IconButton(
      ...
      bottomNavigationBar: state.draft == null ? null : _buildSaveBar(state),
    );
  }
      ...
      child: Center(
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              if (state.error != null) ...[
                _ErrorBanner(message: state.error!, fieldErrors: state.fieldErrors),
          ...
          children: [
            Expanded(
              child: NumberStepperField(
                ...
                onChanged: (v) => _updateDraft(d.copyWith(slotDurationMinutes: v)),
              ),
            ),
            ...
            Expanded(
              child: NumberStepperField(
                ...
                suffix: 'min',
                helper: 'Extra time between appointments for cleaning and prep.',
  ...
  // ─── Section 4: Cancellation & No-Show ────────────────────
    ...
    return SettingsSectionCard(
      ...
      children: [
        NumberStepperField(
          ...
          onChanged: (v) => _updateDraft(d.copyWith(cancellationWindowHours: v)),
        ),
        const SizedBox(height: AppDimensions.paddingMedium),
          ...
          children: [
            Expanded(
              child: FeeField(
                ...
                helper: 'Charged for cancelling after the cutoff. Not yet enforced.',
                value: d.lateCancellationFee,
                errorText: state.fieldErrors['late_cancellation_fee'],
                onChanged: (v) => _updateDraft(d.copyWith(lateCancellationFee: v)),
              ),
            ),
            const SizedBox(width: AppDimensions.paddingMedium),
            Expanded(
              child: FeeField(
                label: 'No-Show Fee',
                helper: 'Charged when a patient does not attend. Not yet enforced.',
                value: d.noShowFee,
        ...
        NumberStepperField(
          ...
          helper: 'Flags or blocks a patient after this many no-shows. Not yet enforced.',
          ...
          children: [
            Expanded(
              child: NumberStepperField(
                ...
                onChanged: (v) => _updateDraft(d.copyWith(firstReminderHours: v)),
              ),
            ),
            const SizedBox(width: AppDimensions.paddingMedium),
            Expanded(
              child: NumberStepperField(
                ...
                max: 720,
                errorText: state.fieldErrors['reminder_offsets'],
                onChanged: (v) => _updateDraft(d.copyWith(secondReminderHours: v)),
              ),
            ),
          ],
        ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                // Inline copies land next to their own fields too; this list
                // catches errors on fields with no inline slot (time pickers).
